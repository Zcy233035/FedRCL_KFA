...
def local_train_net_diagoffline(nets, selected, f_nets, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
...
def train_net_diagoffline(net_id, net, f_local, len_of_data, train_dataloader, test_dataloader, epochs, lr, coordinator, args_optimizer, device="cpu"):
    logger.info('Training network %s' % str(net_id))
...
def local_train_net_diagonline(nets, selected, f_nets, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
...
def train_net_diagonline(net_id, net, f_local, len_of_data, train_dataloader, test_dataloader, epochs, lr, coordinator, args_optimizer, device="cpu"):
    logger.info('Training network %s' % str(net_id))
...
if __name__ == '__main__':
    ...
    if args.alg == 'fedavg':
        ...
        f_nets = [copy.deepcopy(global_model.state_dict()) for i in range(args.n_parties)]
        ...
        for round in range(args.comm_round):
            ...
            local_train_net_diagoffline(nets, selected, f_nets,args, net_dataidx_map, test_dl = test_dl_global, device=device)
            # local_train_net(nets, args, net_dataidx_map, local_split=False, device=device)
        ...
        for round in range(args.comm_round):
            ...
            local_train_net_diagonline(nets, selected, f_nets,args, net_dataidx_map, test_dl = test_dl_global, device=device)
            # local_train_net(nets, args, net_dataidx_map, local_split=False, device=device)
