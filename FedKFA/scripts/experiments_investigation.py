...
def local_train_net_riem(train_net_func, nets, selected, f_nets, args, net_dataidx_map, coordinator, test_dl = None, device="cpu"):
    avg_acc = 0.0
...
def local_train_net_blockadap(nets, selected, f_nets, args, net_dataidx_map, coordinator, test_dl = None, device="cpu"):
    return local_train_net_riem(train_net_blockadap, nets, selected, f_nets, args, net_dataidx_map, coordinator, test_dl, device)
...
def local_train_net_blockonline(nets, selected, f_nets, args, net_dataidx_map, test_dl = None, device="cpu"):
    return local_train_net_riem(train_net_blockonline, nets, selected, f_nets, args, net_dataidx_map, args.coor, test_dl, device)
...
def train_net_blockonline(net_id, net, f_local, len_of_data, train_dataloader, test_dataloader, epochs, lr, coordinator, args_optimizer, device="cpu"):
    ...
    for epoch in range(epochs+1):
        ...
        for name, param in net.named_parameters():
            if len(f_local[name])==2:
                tr1, _ = torch.symeig(f_local[name][0], eigenvectors=True)
                tr2, _ = torch.symeig(f_local[name][1], eigenvectors=True)
                pi = torch.sqrt(tr1.mean()/ tr2.mean())
            else: 
                pi = 1.0
            for f_idx in range(len(f_local[name])):
...
if __name__ == '__main__':
    ...
    if args.alg == 'blockoffline':
        ...
        for round in range(args.comm_round):
            ...
            print('>> feavg Global Model Train accuracy: %f' % train_acc)
            print('>> feavg Global Model Test accuracy: %f' % test_acc)
            ...
            print('>> Unbiased Global Model Train accuracy: %f' % train_acc)
            print('>> Unbiased Global Model Test accuracy: %f' % test_acc)
            ...
            train_acc = compute_accuracy(global_model, train_dl_global)
            test_acc, conf_matrix = compute_accuracy(global_model, test_dl_global, get_confusion_matrix=True)
            print('>> newtwon Global Model Train accuracy: %f' % train_acc)
            print('>> newtwon Global Model Test accuracy: %f' % test_acc)
