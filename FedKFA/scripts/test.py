...
from backpack.extensions import KFAC, KFRA
...
def local_train_net_blockoffline(nets, selected, f_nets, args, net_dataidx_map, test_dl = None, device="cpu"):
    ...
    for net_id, net in nets.items():
        ...
        n_epoch = args.epochs
        trainacc, testacc = train_net_blockoffline(net_id, net, f_nets[net_id], len(dataidxs), train_dl_local, test_dl, n_epoch, args.lr, args.coor, args.optimizer, device=device)
...
def local_train_net_blockadap(nets, selected, f_nets, args, net_dataidx_map, coordinator, test_dl = None, device="cpu"):
    avg_acc = 0.0
    ...
    for net_id, net in nets.items():
        ...
        train_dl_global, test_dl_global, _, _ = get_dataloader(args.dataset, args.datadir, args.batch_size, 32)
        n_epoch = args.epochs
        trainacc, testacc = train_net_blockadap(net_id, net, f_nets[net_id], len(dataidxs), train_dl_local, test_dl, n_epoch, args.lr, coordinator, args.optimizer, device=device)
        logger.info("net %d final test acc %f" % (net_id, testacc))
        avg_acc += testacc
...
def train_net_blockadap(net_id, net, f_local, len_of_data, train_dataloader, test_dataloader, epochs, lr, coordinator, args_optimizer, device="cpu"):
    logger.info('Training network %s' % str(net_id))
                ...
                if epoch<epochs:
                    ...
                    for name, param in net.named_parameters():
                        if batch_idx==0:
                            ...
                            f_local[name] = [(len(target) / len_of_data)*copy.deepcopy(kf_ele)  for kf_ele in param.kfra]
                            # print(name, " fisher", [tem.shape for tem in f_local[name]])
...
def local_train_net_blockonline(nets, selected, f_nets, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
...
def train_net_blockonline(net_id, net, f_local, len_of_data, train_dataloader, test_dataloader, epochs, lr, coordinator, args_optimizer, device="cpu"):
    logger.info('Training network %s' % str(net_id))
    ...
    if args.alg == 'blockadap':
        ...
        for round in range(args.comm_round):
            ...
            if round == 0:
                ...
                for idx in selected:
                    nets[idx].load_state_dict(global_para)
            coordinator = min(1/(round+1), args.coor)
            local_train_net_blockoffline(nets, selected, f_nets,args, net_dataidx_map, test_dl = test_dl_global, device=device)
            # local_train_net_blockadap(nets, selected, f_nets,args, net_dataidx_map, coordinator=coordinator,test_dl = test_dl_global, device=device)
            ...
            tem_global_para = copy.deepcopy(global_para)
            tem_global_fim = copy.deepcopy(global_fim)
            ...
            for idx in range(len(selected)):
                ...
                for key in net_para:
                    if idx == 0 and round == 0:
                        global_fim[key] = [0*tem_lf for tem_lf in f_nets[selected[idx]][key]]
                    ...
                    # f_nets[selected[idx]][key] = [(1-rho)*tem_lf + rho*global_fim[key][tem_idx] for tem_idx, tem_lf in enumerate(f_nets[selected[idx]][key])]
                    # new_param = (1-rho)*new_param + rho*global_para[key].to(device)
            ...
            for key in global_model.state_dict():
                ...
                # global_fim[key] = [(1-rho)*global_fim[key][tem_idx] + rho*tem_lf for tem_idx, tem_lf in enumerate(tem_global_fim[key])]
                # if len(global_fim[key])==2:
                #     # new_param = (new_omega + args.eps)
                #     # +args.eps * torch.eye(new_H.shape[0]).to(device) +args.eps * torch.eye(new_Q.shape[0]).to(device) +args.eps * torch.eye(new_H.shape[0]).to(device)
                #     global_para[key] = (1-rho)*torch.inverse(global_fim[key][0]) @ global_para[key] @ torch.inverse(global_fim[key][1]) + rho*tem_global_para[key].to(device)
                # elif len(global_fim[key])==1:
                #     global_para[key] = (1-rho)*torch.inverse(global_fim[key][0]) @ global_para[key] + rho*tem_global_para[key].to(device)
                if len(global_fim[key])==2:
                    if len(param_shapes[key])>2:
