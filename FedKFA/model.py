...
class SeFcNet(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dims, output_dim, dropout_p=0.0):
        ...
        for i in range(len(self.dims) - 1):
            ...
            op_dim = self.dims[i + 1]
            # modules.append(
            #     nn.Linear(ip_dim+1, op_dim, bias=False)
            #     )
            modules.append(
                nn.Linear(ip_dim, op_dim, bias=True)
                )
    ...
    def forward(self, x):
        ...
        #         add_ones = torch.ones([len(x),1]).to(x.device)
        #         x = torch.cat([x, add_ones], dim=1)
        #     x = layer(x)
        return x

class SeFcNetBN(nn.Module):
    """
    Fully connected network for MNIST classification with BatchNorm Layer
    """
    ...
    def __init__(self, input_dim, hidden_dims, output_dim, dropout_p=0.0):
        ...
        for i in range(len(self.dims) - 1):
            ...
            if i < (len(self.dims) - 2):
                modules.append(nn.BatchNorm1d(op_dim))
                modules.append(
                    nn.ReLU()
                    )
                modules.append(
...
class SimpleCNN(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim=10):
        ...
        self.layers = nn.ModuleList([
            nn.Conv2d(3, 6, 5), nn.MaxPool2d(2, 2),
            nn.Conv2d(6, 16, 5), nn.MaxPool2d(2, 2), nn.Flatten(),
...
class SimpleCNNMNIST(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim=10):
        ...
        self.layers = nn.ModuleList([nn.Conv2d(1, 6, 5),nn.ReLU(),nn.MaxPool2d(2,2),
